...
System Settings -> Access now owns the system-login access UI copied from the old `/home/threejmon` System Settings -> Access flow: Auth Settings, Permissions, Roles, and Users. API routes live under `/api/system-settings/access`. Access data persists to `SYSTEM_SETTINGS_DATA_PATH`, and app-shell login now accepts Access users while keeping the legacy `admin` fallback. Access seeds Tech Portal permission codes, a built-in `technician` role, and temporary test user `tech` / `tech12345`; full per-module permission enforcement beyond the first Tech Portal UI restriction is still future work.
...
The shared API runs PostgreSQL migrations on startup when `DATABASE_URL` is configured and records applied migrations in `schema_migrations`. Current migration status is available at `/api/system/database-migrations` for authenticated admins. Customer Profiling real-data Stage 2 uses migration `2026052601_customer_profiles` to create the durable `customer_profiles` table. Billing uses migration `2026071001_billing_records` to create the durable `billing_records` table for module records. Service uses migration `2026071002_service_records` to create the durable `service_records` table for Service Catalog, Service Account, and Service Order records. Docker Compose sets `CUSTOMER_PROFILING_STORAGE=postgres` by default and `CUSTOMER_PROFILING_SEED_DEMO=false` so new deployments start empty for real customer entry instead of demo records. Billing and Service default to PostgreSQL when `DATABASE_URL` is configured; Billing demo seed records stay disabled unless `BILLING_SEED_DEMO=true`, while Service keeps only its baseline catalog seed enabled unless `SERVICE_SEED_CATALOG=false`.
