...
import { PostType, profiledataType, Reply, Comment } from 'src/types/apps/userprofile';

import React from 'react';
import useSWR from 'swr';
import { getFetcher, postFetcher } from 'src/api/global-fetcher';
import { GallaryType, userType } from 'src/types/apps/users';
...
export const UserDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const {
    ...
    error: postsError,
    mutate,
  } = useSWR('/api/data/postData', getFetcher);
  const {
    data: usersData,
    isLoading: isUsersLoading,
    error: usersError,
  } = useSWR('/api/data/users', getFetcher);
  const {
    data: galleryData,
    isLoading: isGalleryLoading,
    error: galleryError,
  } = useSWR('/api/data/gallery', getFetcher);
  ...
  const filterFollowers = () => {
    if (followers) {
      return followers.filter((t) => t.name.toLowerCase().includes(search.toLowerCase()));
    }
    return followers;
  };
  ...
  const addComment = async (postId: string, comment: Comment) => {
    try {
      await mutate(
        postFetcher('/api/data/posts/comments/add', {
          postId,
          comment,
        }),
      );
  ...
  const addReply = async (postId: string, commentId: string, reply: Reply) => {
    try {
      await mutate(
        postFetcher('/api/data/posts/replies/add', {
          postId,
          commentId,
          reply,
        }),
