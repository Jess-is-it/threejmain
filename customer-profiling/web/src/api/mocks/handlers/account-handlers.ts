...
export const accountHandlers = [
  ...
  http.post('/api/account/register', async ({ request }) => {
    try {
      ...
      const { email, password, username } = (await request.json()) as RegisterRequestBody;
      ...
      user = {
        id: uid,
        displayName: username || email, // Use username if provided, fallback to email
        email,
        password,
        phoneNumber: '',
        country: '',
      ...
      users.push(user); // Optionally push new user to array to simulate DB
