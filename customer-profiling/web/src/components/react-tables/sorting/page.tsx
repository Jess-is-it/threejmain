...
const columns = [
  ...
  columnHelper.accessor('status', {
    cell: (info) => (
      ...
      >
        {info.row.original.statusicon && <info.row.original.statusicon size={15} />}
    ...
    cell: (info) => (
      <div className=" text-sm flex items-center gap-3">
        <div className="w-full">
          <Progress value={info.getValue()} className="w-full" />
        </div>
        {info.getValue()}%
      </div>
    ),
...
const SortingTable = () => {
  ...
  const handleDownload = () => {
    ...
    const rows = data.map((item) => [item.invoice, item.status, item.name, item.progress]);
