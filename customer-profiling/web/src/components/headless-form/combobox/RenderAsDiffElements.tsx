...
const MyCustomButton = forwardRef<HTMLButtonElement, MyCustomButtonProps>((props, ref) => {
...
const RenderingCombobox = () => {
  ...
  return (
    <CardBox>
      <h4 className="text-lg font-semibold mb-2">Rendering As Different Elements</h4>
      <Combobox
        value={selectedPerson}
        onChange={(person: Person | null) => setSelectedPerson(person)}
        onClose={() => setQuery('')}
      >
        <span className="flex gap-3">
          ...
          />
          <ComboboxButton as={MyCustomButton} className="ui-button bg-primary text-white">
            Open
          </ComboboxButton>
        </span>
        <ComboboxOptions
