    ...
    <div className="ticketing-page">
      ...
      <div className="row row-cards">
        <div className="col-lg-7">
          <Card title="Tickets" icon={IconTicket} actions={<button className="btn btn-primary" type="button" onClick={resetForm}><IconPlus size={16} /> New</button>}>
            <form className="row g-2 mb-3" onSubmit={(event) => { event.preventDefault(); load(filters); }}>
              ...
              <div className="col-md-2">
                <select className="form-select" value={filters.status} onChange={(event) => setFilters({ ...filters, status: event.target.value })}>
                  ...
                  {meta.statuses.map((status) => <option key={status} value={status}>{label(status)}</option>)}
                </select>
              </div>
              <div className="col-md-2">
            ...
            <div className="ticketing-list">
              {tickets.map((ticket) => (
                <div className={`ticketing-row ${selectedId === ticket.id ? 'active' : ''}`} key={ticket.id}>
                  <div className="d-flex justify-content-between gap-2">
                    <button className="btn btn-link p-0 text-start" type="button" onClick={() => editTicket(ticket)}>
                      <div className="ticketing-subject">{ticket.ticketNumber} - {ticket.subject}</div>
                  ...
                  <div className="ticketing-badges mt-2">
                    <span className={`badge ${statusClass(ticket.status)}`}>{label(ticket.status)}</span>
                    <span className={`badge ${statusClass(ticket.priority)}`}>{label(ticket.priority)}</span>
