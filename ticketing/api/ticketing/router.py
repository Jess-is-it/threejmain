...
def apply_status_dates(ticket: dict[str, Any], previous_status: str | None = None) -> None:
    ...
    if status not in CLOSED_STATUSES:
        ticket["resolvedAt"] = "" if previous_status in CLOSED_STATUSES else ticket.get("resolvedAt", "")
        ticket["closedAt"] = ""


def ticket_matches(ticket: dict[str, Any], search: str = "", status: str = "", priority: str = "", category: str = "", customer_id: str = "", assigned_to: str = "") -> bool:
    ...
    if search:
        ...
        haystack = " ".join(
            ...
            for field in ["ticketNumber", "subject", "description", "requestorName", "contactNumber", "assignedTo", "serviceId", "outageId"]
        ).lower()
        customer = ticket.get("customer") or {}
        haystack = f"{haystack} {customer.get('accountNumber', '')} {customer.get('name', '')}".lower()
...
def seed_ticketing_data() -> None:
    ...
    second_customer = customers[1] if len(customers) > 1 else None
    samples = [
        {
        ...
        {
            "customerId": second_customer["id"] if second_customer else None,
            "requestorName": second_customer["name"] if second_customer else "Barangay office",
            "contactNumber": second_customer["contactNumber"] if second_customer else "09171111111",
            "subject": "New install needs field visit",
            "description": "Customer is ready for installation scheduling.",
            "category": "INSTALLATION",
            "priority": "NORMAL",
            "status": "IN_PROGRESS",
            "source": "INTERNAL",
            "assignedTo": "Field Team",
            "serviceId": "SERVICE-PLACEHOLDER-002",
            "dueDate": "",
        },
    ]
...
def ticketing_meta(admin=Depends(require_admin)):
    return {
        ...
        "placeholders": {
            "customerLookup": "Uses Customer Profiling when available; requestor fields allow manual tickets until customer linking is complete.",
            "assignedTo": "Free-text assignee until Account Admin staff records are available.",
...
def ticketing_overview(admin=Depends(require_admin)):
    ...
    rows = visible_tickets()
    open_rows = [ticket for ticket in rows if ticket.get("status") not in CLOSED_STATUSES]
    by_status = {status: len([ticket for ticket in rows if ticket.get("status") == status]) for status in TICKET_STATUSES}
    by_priority = {priority: len([ticket for ticket in open_rows if ticket.get("priority") == priority]) for priority in TICKET_PRIORITIES}
    recent = sorted(rows, key=lambda ticket: ticket.get("updatedAt", ""), reverse=True)[:6]
    return {"metrics": ticketing_metrics(), "byStatus": by_status, "byPriority": by_priority, "recentTickets": recent}
