...
- `POST /api/billing/payments` to post invoice payments with `collectionChannel=POS`
...
- Invoice Payments: POS-owned customer invoice settlement workspace. It reads Billing invoices, posts Billing payment records, enforces invoice-balance payment limits in the UI, and keeps invoice payment activity separate from POS retail sales.
...
- Billing: canonical invoice and billing-payment ledger. POS reads Billing invoices and posts Billing payment records for customer invoice settlement.
...
- `Invoice Payments` is the customer billing payment desk. It has open invoice KPIs, a searchable payable invoice queue, and selected-invoice payment capture. Receipt history is intentionally not duplicated here.
