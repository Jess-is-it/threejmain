...
- The dedicated Collections tab owns period controls and the Collection Worklist customer search, status filters, rows, and pagination. It defaults to `ACTION_REQUIRED`, which combines partially paid and unpaid selected-month customers, ranks overdue balances first, and shows due context, monthly bills, cash/account-credit settlement, rebates, and balance. Clicking Partially Paid or Unpaid on Overview opens the matching Collections filter. Fully paid customer detail is reserved for future reporting. The legacy `GET /api/billing/overview` `collectionAccounts` fields remain available for compatibility, but the Overview no longer renders customer worklists, Recent Invoices, or Collection Risk tables.
