...
- PLC/LCP output handling in the NAP/Junction splitter modal is port-based: each 1x4, 1x8, or 1x16 output port renders its own wire line and connector terminal (`port1`, `port2`, etc.) so operators can attach downstream splitter objects or mark individual customer house drops. FBT output handling exposes OUT actions on both Splitter A and Splitter B branches. IN remains single per container, while multiple valid OUT/customer-drop endpoints can be marked when the physical splitter has multiple usable outputs.
