...
Network Settings is the ISP network source-of-truth module for access network assets, starting with device records, SNMP capture runs, MikroTik PPPoE account discovery, OLTs, generated PON ports, captured ONU inventory, NAP boxes, PLC/LCP/FBT splitter catalog records, and network topology.
...
| Insertion Loss / Splitters | PLC, LCP, and FBT splitter CRUD by manufacturer/model. PLC and LCP records can contain multiple selected ratio profiles, limited to 1:4, 1:8, and 1:16, with insertion loss tracked per output port for each selected ratio. Network Topology lists each selected PLC/LCP ratio under the same manufacturer/model. FBT records auto-populate preset ratio rows and let operators add custom ratios with connector/deployment NAP/next NAP loss entered as comma- or slash-separated values for selected 1310, 1490, and/or 1550 nm wavelengths. Compatibility route remains `/network-settings/fbts`. |
...
The Mapping page shows OLT markers plus only NAP boxes that have been placed in Network Topology. NAP links are derived from the Network Topology source path: PON-sourced NAPs draw from the OLT marker, and downstream NAPs draw from the mapped upstream NAP when available. Mapping, Serviceability Check, and the OLT coordinate capture picker use the shared System Settings -> Maps provider configuration, including enabled provider selection, provider max zoom, fallback built-in Esri/OpenStreetMap providers, and Google Map Tiles providers that create a Google session token before loading 2D tiles. Serviceability Check can also be opened directly from Customer Profiling with `?customerId=<id>` and then shows only that customer in the split customer/map view. NAP marker images have rounded corners and inherit the assigned parent PON color as their stroke. A Move markers switch lets operators drag visible OLT/NAP markers; dropping a marker converts the mapping screen point into latitude/longitude and saves those coordinates back to the OLT or NAP record.

The Network Topology page lives directly under Network Settings, below Mapping and above the MikroTik group. It now renders a fixed-row TreeView with one tab per OLT: each active OLT tab shows the OLT root row, its PON rows, and mapped NAP/Junction rows indented under their saved `sourceKey`. The main topology view has no drag movement, zoom surface, inline NAP expansion, or resizable objects, so branches cannot overlap visually. A bendable SVG connector layer draws parent-child lines without arrowheads from the saved source row to the child row, including direct NAP-to-NAP links. Only PON rows expose the Fiber Link action. Clicking it opens a PON-level fiber-link canvas that displays the selected PON plus its mapped NAP/Junction topology, with only fiber line segments clickable; selecting a segment opens the Fiber Link modal for fiber profile, fiber core, length, source power, losses, and line style, and closing that Fiber Link modal returns to the PON canvas. The PON canvas supports pan/zoom on a grid background like the NAP Splitter Layout, uses the PON icon and configured NAP marker image, and lets operators click each line-end connector to cycle Fusion, Mechanical, and SC Connector shapes directly on the canvas. Changing the PON segment-end connector syncs the first splitter input connector for that NAP/Junction, and changing that first input connector in the splitter layout syncs back to the PON segment. Operators can click NAP/Junction nodes inside the PON canvas to open that container's splitter layout as a nested editor; the splitter modal shows the incoming PON fiber context and a feeder line entering the input side so splitter chains can be connected to the upstream segment. Saved segment lengths resize the PON canvas links against the longest saved segment in that PON, with max/min line sizes managed from Fiber Settings inside the PON Fiber Link modal; visible line labels show the fiber/core and total distance rather than the internal render-size percentage. Fiber link colors are derived from the selected Fiber Optic core color instead of a manual line-color field. PON/NAP/Junction rows still expose Tabler-style action badges for adding an assigned NAP or Junction Box, opening the splitter modal, or removing a mapped NAP/Junction branch. Removing a mapped NAP/Junction also removes its downstream Network Topology branch so child objects are not orphaned. Splitter editing for NAP and Junction containers happens in the larger modal layout surface instead of expanding the row in the main TreeView. The old splitter dropdown and chip tags are hidden for now; connector plus buttons in the modal open a splitter picker for type, maker/model, and ratio/output selection, while input connector ends do not show a plus button. Internal splitter circles can be clicked to select, removed with Delete/Backspace, or removed through the hover `-` control. Removing a splitter removes only that splitter; children are detached so their connector ends remain available for re-joining. Child splitters spawn from the clicked output connector without moving the parent splitter, with a short branch-growth animation. Curved SVG fiber paths connect the internal input, splitter circles, split branches, and output connector dots inside the modal. PLC/LCP equipment renders its incoming branch at the same length as the output branch so the input connector stays outside the SVG body when connected to the incoming fiber. Connector dots expose details on hover/click and cycle the connection label and shape: Fusion is an orange oval, Mechanical is a black rectangle, and SC Connector is a blue rectangle. OLT/NAP artwork uses the marker images configured in System Settings -> Images, while PON/Junction and splitter placeholders use icons. The toolbar includes Reset Topology to remove added NAP mapping, Junction Boxes, splitter assignments, saved link settings, and custom node positions so only OLT and PON rows remain. Network Topology success messages render as centered TreeView toasts that auto-hide after six seconds. The implementation persists edge settings, PON fiber link settings, NAP splitter assignments, Junction Boxes, container splitter assignments, per-instance splitter assignments including optional parent-terminal attachment, and connection-point labels/endpoint positions in Network Settings JSON data through the compatibility `GET/PATCH /api/network-settings/fiber-mapping` endpoint; saved main-node `x/y` and lock values are retained only for backward compatibility.

Network Topology mini-canvas note: expanded NAP/Junction splitter internals now use flexible curved SVG paths, click-to-select splitter circles with Delete/Backspace or hover `-` removal, non-cascading splitter deletion, draggable connector/branch endpoints with output-to-input auto-attach, and a larger modal layout surface opened from the NAP/Junction object. The modal mini-canvas uses the same coordinate metrics as the inline mini-canvas so connector dots, SVG paths, and splitter circles stay attached. The old summary line, splitter dropdown, fixed left/right IN/OUT targets, and splitter chip tags are intentionally hidden for now; connector plus controls open the compact anchored splitter popup, add splitters to the clicked output terminal, fall back to the other FBT branch when the requested branch is occupied, keep the parent splitter fixed, and animate the new branch/splitter spawn. New splitters start with detached input/output dots for custom line shaping, but IN/OUT assignment is primarily handled through delayed-hide splitter hover actions: the first splitter toggles `Set as IN` / `Remove IN`, the last splitter toggles `Set as OUT` / `Remove OUT`, and the IN/OUT badge renders directly on that connector endpoint.

Topology splitter output note: FBT splitters expose OUT actions on both Splitter A and Splitter B branches. LCP splitters expose one separated connector line per physical output port (`port1`, `port2`, etc.) for 1x4, 1x8, and 1x16 ratios, and each visible LCP port can add a downstream splitter or be marked with a customer-house drop icon. PLC splitters keep one topology output connector, while their catalog records can still keep per-port insertion-loss values. IN remains a single container input, while LCP and FBT can keep multiple valid OUT/customer-drop endpoints.
...
- PON port records: create, update, list, archive, link to OLT, track display color, capacity, split ratio, service VLANs, optical thresholds, and operational status. Default PON colors use `#20C997`, `#FCC419`, `#339AF0`, and `#CC5DE8` for the first four PONs, with deterministic variants for higher port numbers.
...
- `NetworkDevice`: MikroTik or OLT endpoint, access method (`API` or `SNMP`), management host, API username, displayed MikroTik API password, non-secret profile names, status, and optional linked OLT inventory record. Site / Location is not currently operator-entered in the device form.
...
- MikroTik API device passwords are intentionally returned in the MikroTik API device response for operator visibility. SNMP credentials and PPPoE passwords should still not be returned in normal API responses.
