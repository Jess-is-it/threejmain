...
Topology splitter output note: FBT splitters expose OUT actions on both Splitter A and Splitter B branches. PLC and LCP splitters expose one connector line per physical output port (`port1`, `port2`, etc.) for 1x4, 1x8, and 1x16 ratios, and each visible port can add a downstream splitter or be marked with a customer-house drop icon. IN remains a single container input, while multi-output splitters can keep multiple valid OUT/customer-drop endpoints.
