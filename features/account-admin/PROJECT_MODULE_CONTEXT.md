...
Current Phase 1 UI is intentionally table-only:

- One Customer Account row is created for every visible Customer Profile.
- The page shows a single Customer Accounts table.
- Search, filter toggle, and tabs follow the Customer Profiling table pattern.
- `All`, `Customer w/ Tickets`, and temporary `PPPoE & ONUs` tabs are shown.
- Tabs include badge counts.
...
- PPPoE account/status, router, and assigned IP are displayed when a binding exists.
- The temporary `PPPoE & ONUs` tab reads MikroTik PPPoE accounts and OLT ONUs from Network Settings, lists every discovered PPPoE account, and shows the matched captured ONU when one exists.
