...
- Branding asset upload endpoints are `PUT /api/system-settings/branding/company-logo` and `PUT /api/system-settings/branding/browser-logo`. Public asset routes are `/api/public/branding/company-logo` and `/api/public/branding/browser-logo`, while `/api/public/branding` returns sanitized display fields and cache-busted asset URLs for the app shell.
